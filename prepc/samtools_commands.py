...
def samtools_index_ref(in_ref, log_dir):
    ...
    stderr_file = open(log_dir + "/samtools_index_ref" +
                       time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')
    out_file = open(
        log_dir + "/samtools_index_ref" + time.strftime("-%Y-%m-%d-%H-%M-%S.log"), 'w')
...
def samtools_sam_to_bam(in_sam, out_bam, log_dir):
    ...
    stderr_file = open(log_dir + "/samtools_sam_to_bam" +
                       time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')
...
def samtools_bam_sort(in_bam, out_bam, temp, log_dir):
    ...
    stderr_file = open(
        log_dir + "/samtools_bam_sort" + time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')
...
def samtools_bam_index(in_bam, log_dir):
    ...
    # log files for standard out and error
    log_file = open(
        log_dir + "/samtools_bam_index" + time.strftime("-%Y-%m-%d-%H-%M-%S.log"), 'w')
    stderr_file = open(log_dir + "/samtools_bam_index" +
                       time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')

    # run command
    subprocess.call(
        ["samtools", "index", in_bam], stdout=log_file, stderr=stderr_file)

    # checking output
    index_file = in_bam + ".bai"
    assert os.path.isfile(
        index_file), "Expected output file %s not found" % index_file
...
def samtools_bam_group_sort(in_bam, out_bam, temp, log_dir):
    ...
    # prep files
    log_file = open(log_dir + "/samtools_bam_group_sort" +
                    time.strftime("-%Y-%m-%d-%H-%M-%S.log"), 'w')
    stderr_file = open(log_dir + "/samtools_bam_group_sort" +
                       time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')

    # run command
    bam_group_sort_command = [
        "samtools", "sort", "-n", "-O", "bam",
        "-o", out_bam, "-T", temp, in_bam]
    subprocess.call(
        bam_group_sort_command, stdout=log_file, stderr=stderr_file)
...
def samtools_bam_fixmate(in_bam, out_bam, log_dir):
    ...
    # log files for standard out and error
    log_file = open(log_dir + "/samtools_bam_fixmate" +
                    time.strftime("-%Y-%m-%d-%H-%M-%S.log"), 'w')
    stderr_file = open(log_dir + "/samtools_bam_fixmate" +
                       time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')
...
def samtools_bam_merge(bam_list, out_bam, log_dir):
    ...
    # prep files
    log_file = open(
        log_dir + "/samtools_bam_merge" + time.strftime("-%Y-%m-%d-%H-%M-%S.log"), 'w')
    stderr_file_name = log_dir + "/samtools_bam_merge" + \
    ...
    # checking output
    assert os.path.isfile(out_bam), "Expected output file %s not found. See stderr file %s" % (
...
def samtools_mpileup(in_ref, in_bams, out_vcf, log_dir, varscan_pairs=False):
    ...
    stderr_file = open(
        log_dir + "/samtools_mpileup" + time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')
...
def samtools_mpileup_pairs(in_ref, in_bams, out_mpileup, log_dir):
    ...
    stderr_file = open(log_dir + "/samtools_mpileup_pairs" +
                       time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')

    # run command
    mpileup_command_pairs = [
        "samtools", "mpileup", "-q", "1", "-f", in_ref] + in_bams
    subprocess.call(
        mpileup_command_pairs, stdout=mpileup_file, stderr=stderr_file)
...
def samtools_mpileup_single(in_ref, in_bam, out_mpileup, log_dir):
    ...
    # run command
    mpileup_command_pairs = [
        "samtools", "mpileup", "-q", "1", "-f", in_ref, in_bam]
    subprocess.call(
        mpileup_command_pairs, stdout=mpileup_file, stderr=stderr_file)
...
def samtools_depth(in_bam, out_depth, log_dir):
    ...
    stderr_file_name = log_dir + "/samtools_depth" + \
        time.strftime("-%Y-%m-%d-%H-%M-%S.stder")
    stderr_file = open(stderr_file_name, 'w')
