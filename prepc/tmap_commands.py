...
def tmap_index_ref(in_ref, log_dir):
    ...
    # prep files
    log_file_name = log_dir + "/tmap_index_ref" + \
        time.strftime("-%Y-%m-%d-%H-%M-%S.log")
    log_file = open(log_file_name, 'w')
    stderr_file_name = log_dir + "/tmap_index_ref" + \
    ...
    subprocess.call(
        tmap_index_ref_command, stdout=log_file, stderr=stderr_file)
...
def tmap_map_fq(in_ref, in_fq, out_sam, log_dir):
    ...
    # prep files
    log_file_name = log_dir + "/tmap_map" + \
        time.strftime("-%Y-%m-%d-%H-%M-%S.log")
    log_file = open(log_file_name, 'w')
    stderr_file_name = log_dir + "/tmap_map" + \
        time.strftime("-%Y-%m-%d-%H-%M-%S.stder")
    stderr_file = open(stderr_file_name, 'w')

    # run command
    tmap_map_command = ["tmap", "mapall", "-n", num_cores,
                        "-f", in_ref, "-r", in_fq,
                        "-v", "-Y", "-s", out_sam, "stage1", "map4"]
