...
def picard_create_dict(in_ref, out_dict, log_dir):
    ...
    # run command
    markdup_command = ["java","-Xmx2g","-jar","/usr/local/bin/CreateSequenceDictionary.jar",
...
def picard_index_stats(in_bam, log_dir):
    ...
    # run command
    index_stats_command = ["java","-Xmx2g","-jar","/usr/local/bin/BamIndexStats.jar",
                        ("I=%s" % (in_bam))]
...
def picard_alignment_metrics(in_bam, bam_stats, log_dir):
    ...
    # run command
    alignment_stats_command = ["java","-Xmx2g","-jar","/usr/local/bin/CollectAlignmentSummaryMetrics.jar",
...
def picard_multiple_metrics(in_bam, bam_stats, log_dir):
    ...
    multiple_metrics_command = ["java","-Xmx2g","-jar","/usr/local/bin/CollectMultipleMetrics.jar",
