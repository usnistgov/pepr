...
def picard_create_dict(in_ref, out_dict, log_dir):
    ...
    # run command
    markdup_command = ["java","-Xmx8g","-jar","/usr/local/bin/picard.jar", "CreateSequenceDictionary",
...
def picard_add_header(in_bam, out_bam, log_dir, read_group):
    ...
    # run command
    add_header_command = ["java","-Xmx8g","-jar","/usr/local/bin/picard.jar","AddOrReplaceReadGroups",
...
def picard_markdup(in_bam, out_bam, metrics_file, log_dir):
    ...
    # run command
    markdup_command = ["java","-Xmx8g","-jar","/usr/local/bin/picard.jar", "MarkDuplicates","VALIDATION_STRINGENCY=LENIENT",
...
def picard_index_stats(in_bam, log_dir):
    ...
    # run command
    index_stats_command = ["java","-Xmx8g","-jar","/usr/local/bin/picard.jar", "BamIndexStats",
                        ("I=%s" % (in_bam))]
...
def picard_alignment_metrics(in_bam, bam_stats, log_dir):
    ...
    # run command
    alignment_stats_command = ["java","-Xmx8g","-jar","/usr/local/bin/picard.jar","CollectAlignmentSummaryMetrics",
...
def picard_multiple_metrics(in_bam, bam_stats, log_dir):
    ...
    multiple_metrics_command = ["java","-Xmx8g","-jar","/usr/local/bin/picard.jar", "CollectMultipleMetrics",
