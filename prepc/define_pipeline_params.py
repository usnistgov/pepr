...
def move_ref_to_ref_dir(ref, name, analysis_params):
    ''' function for copying reference seqeunce to the reference
        directory and adding it to analysis_params
    ...
    analysis_params['ref_dict'] = analysis_params[
        'ref_dir'] + "/" + analysis_params['ref_root'] + ".dict"
...
def init_prj(pipeline_params, move_ref=True):
    '''initiating project generates directory structure,
    copies ref to directory, and initiates the analysis_params dictionary
...
def init_params(pipeline_params, analysis_params):
    '''generating a list of accession numbers for get_fastq
    and initiating data set specific run params
    @param pipeline_params dictionary generated from pipeline params yaml file
    ...
    for i in pipeline_params['exp_design']:
        ...
        for j in pipeline_params['exp_design'][i]:
            ...
            lib_name = "%d-%d" % (analysis_params[acc]['vial'],
                                  analysis_params[acc]['rep'])
            analysis_params[acc]['lib'] = lib_name
...
def init_analysis(analysis_name, analysis_params, run_by):
    ...
    analysis_root = analysis_params[
        'prj_dir'] + "/" + analysis_params['ref_root'] + "_" + analysis_name
...
def define_map_run(accession, analysis_params, pipeline_params):
    ...
    analysis_params[accession][
        'group_sort_file'] = tmp_root + "_group_sort.bam"
    ...
    if analysis_params[accession]['plat'] == "pacbio":
        ...
        analysis_params[accession][
            'markdup_file'] = output_root + "_refined.bam"
    rg_def = [(("RGID=%s") % pipeline_params['project_id']),
              (("RGLB=%s") % analysis_params[accession]['lib']),
              (("RGPL=%s") % analysis_params[accession]['plat']),
              (("RGPU=%s") % 'barcoded'),
              (("RGSM=%s") % accession),
              (("RGCN=%s") % pipeline_params['center'])]
    analysis_params[accession]['read_group'] = rg_def
...
def define_pilon_run(plat, analysis_params):
    ...
    analysis_params[plat]['pilon_bam_list'] = analysis_params[
        'pilon']['tmp_dir'] + "/" + root_name + "_bam_list.txt"
    analysis_params[plat]['pilon_merged_bam'] = analysis_params[
        'pilon']['tmp_dir'] + "/" + root_name + "_merged.bam"
    analysis_params[plat]['pilon_root'] = analysis_params[
        'pilon']['analysis_dir'] + "/" + root_name
...
def define_qc_run(accession, analysis_params):
    ...
    @params analysis_params
    '''
    root_name = analysis_params['qc_stats'][
        'analysis_dir'] + "/" + analysis_params['ref_root'] + "_" + accession
...
def define_consensus_base_run(plat, analysis_params):
    ...
    # output files
    output_root = analysis_params['consensus_base'][
        'analysis_dir'] + "/" + root_name
...
def define_homogeneity_run(accession1, accession2, analysis_params):
    ...
    root_name = analysis_params['ref_root'] + \
        "_" + accession1 + "-" + accession2

    # input files
    analysis_params[pair_name]['bam1_file'] = analysis_params[
        accession1]['markdup_file']
    analysis_params[pair_name]['bam2_file'] = analysis_params[
        accession2]['markdup_file']

    # temp files
    tmp_root = analysis_params['homogeneity']['tmp_dir'] + "/"
    analysis_params[accession1][
        'mpileup_file'] = tmp_root + accession1 + ".mpileup"
    analysis_params[accession2][
        'mpileup_file'] = tmp_root + accession2 + ".mpileup"
    analysis_params[pair_name]['mpileup_file1'] = analysis_params[
        accession1]['mpileup_file']
    analysis_params[pair_name]['mpileup_file2'] = analysis_params[
        accession2]['mpileup_file']

    # output files
    output_root = analysis_params['homogeneity'][
        'analysis_dir'] + "/" + root_name
    analysis_params[pair_name][
        'varscan_snp_file'] = output_root + "_varscan-snp.txt"
    analysis_params[pair_name][
        'varscan_indel_file'] = output_root + "_varscan-indel.txt"
...
def define_pathoscope_run(accession, analysis_params):
    ...
    if analysis_params[accession]['fastq2'] is not None:
        analysis_params[accession][
            'trimmed_fastq1'] = tmp_root + accession + '_1_qc.fq.gz'
        analysis_params[accession][
            'trimmed_fastq2'] = tmp_root + accession + '_2_qc.fq.gz'
    else:
        analysis_params[accession][
            'trimmed_fastq1'] = tmp_root + accession + '_qc.fq.gz'
        analysis_params[accession]['trimmed_fastq2'] = None

    # output files
    analysis_params[accession]['pathoscope_run_id'] = analysis_params[
        'ref_root'] + "_" + accession
    analysis_params[accession]['pathomap_sam'] = tmp_root + "/" + accession + \
        "/" + \
        analysis_params[accession]['pathoscope_run_id'] + "-appendAlign.sam"
