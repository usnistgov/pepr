...
def pathoqc_command(plat, fastq1, log_dir, out_dir, thread_num, fastq2=False):
    ...
    log_file = open(
        log_dir + "/pathoqc" + time.strftime("-%Y-%m-%d-%H-%M-%S.log"), 'w')
    stderr_file = open(
        log_dir + "/pathoqc" + time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')

    pathoqc_command = ["python", "/PathoScope/pathoqc_v0.1.2/pathoqc.py"]

    if fastq2 != None:
        pathoqc_command = pathoqc_command + \
            ['-1', fastq1, '-2', fastq2, '-s', plat,
                '-p', str(thread_num), '-o', out_dir]
    else:
        pathoqc_command = pathoqc_command + \
            ['-1', fastq1, '-s', plat, '-p', str(thread_num), '-o', out_dir]
...
def pathomap_command(ref_path, index_dir, fastq1, log_dir, out_dir, out_sam, thread_num, exptag, fastq2=False):
    # log file stores standard out
    log_file = open(
        log_dir + "/pathomap" + time.strftime("-%Y-%m-%d-%H-%M-%S.log"), 'w')
    stderr_file = open(
        log_dir + "/pathomap" + time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')

    # pathoscope command root
    pathomap_command = ["python", "/PathoScope/pathoscope/pathoscope.py", '--verbose', 'MAP', '-numThreads', thread_num, '-targetRefFiles', ref_path,
...
def pathoid_command(input_sam, log_dir, out_dir, exptag):
    ...
    # log file stores standard out
    log_file = open(
        log_dir + "/pathoid" + time.strftime("-%Y-%m-%d-%H-%M-%S.log"), 'w')
    stderr_file = open(
        log_dir + "/pathoid" + time.strftime("-%Y-%m-%d-%H-%M-%S.stder"), 'w')
