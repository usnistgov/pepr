...
def main(analysis_params, refine=False):
    ...
    for i in analysis_params['accessions']:
        ...
        if analysis_params[i]['plat'] not in ['pgm', 'miseq', 'pacbio']:
            message = "Accession %s not run, only accessions with plat values 'pgm','miseq'. 'pacbio' are run" % (
                i)
    ...
    Parallel(n_jobs=job_num)(
        delayed(sam_to_bam)
        (i, analysis_params[i]) for i in sam_to_bam_accessions)
    ...
    Parallel(n_jobs=job_num)(delayed(sam_to_bam)(
        i, analysis_params[i]) for i in pacbio_sam_to_bam_accessions)
    ...
    if refine:
        ...
        Parallel(n_jobs=num_cores)(delayed(refine_bam_pipeline)(
            i, analysis_params['ref'], analysis_params[i]) for i in refine_accessions)
