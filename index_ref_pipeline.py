...
import os
from prepc.bwa_commands import bwa_index_ref
from prepc.tmap_commands import tmap_index_ref
from prepc.picard_commands import picard_create_dict
from prepc.samtools_commands import samtools_index_ref
...
def main(analysis_params):
    ...
    # tmap index
    indexed_refs = [analysis_params['ref'] + ".tmap." +
                    i for i in ['anno', 'bwt', 'pac', 'sa']]
    ...
    # bwa index
    indexed_refs = [analysis_params['ref'] +
                    i for i in ['.amb', '.ann', '.bwt', '.pac', '.sa']]
    index_check = map(os.path.isfile, indexed_refs)
    if False in index_check:
